...
n("cv-installer-11", "cv-deploy", "file", "Instaladores 1.1 lab + posto",
  "NSIS dual: Veyon-CIMED-1.1-win64-setup.exe (Master ON) e -posto-setup.exe (Master OFF) — gerados 27/08/2026.",
  "installer/veyon-cimed.nsi",
  "WriteRegStr HKLM ... VEYON_SHARED_CONNECTION_STATE 1\nnetsh advfirewall ... UDP 11777",
  ["DisplayName UI 1.1; sentinela token VERSÃO 1.0.", "install_distributefiles_guard.ps1 embutido.", "GPO: setup.exe /S", "Release tag veyon-cimed-1.0-win64-20260626 recebe assets 1.1 adicionais."]),

n("cv-lab-scripts", "cv-root", "module", "Scripts de laboratório piloto",
  "Deploy, diagnóstico e restore entre .86 e .119 — sem alterar o IPC Fechar nem AccessLogWriter.",
  "scripts/",
  "deploy_filetransfer_fix_86_119.ps1\ntest_hover_stack_ipc.py\nrestore_ponto_funcional_todas_telas.ps1",
  ["Deploy FT e multimonitor entre hosts piloto.", "Simulação de layout de 10 hovers.", "Restore points: todas telas / multimonitor."]),

n("cv-lab-ft-deploy", "cv-lab-scripts", "file", "deploy_filetransfer_fix_86_119.ps1",
  "Deploy cauteloso do fix File Transfer entre 186.217.65.86 e .119.",
  "scripts/deploy_filetransfer_fix_86_119.ps1",
  "powershell -File scripts\\deploy_filetransfer_fix_86_119.ps1",
  ["Copia binários FT/core com backup.", "Reinicia VeyonService nos dois lados.", "Validar FT bidirecional via Master UI."]),

n("cv-lab-hover-test", "cv-lab-scripts", "file", "test_hover_stack_ipc.py",
  "Testa IPC da pilha de hovers (magic 0x56545934) sem abrir Master.",
  "scripts/test_hover_stack_ipc.py",
  "python scripts/test_hover_stack_ipc.py",
  ["Simula Add/Remove outbound.", "Útil para regressão do tray-hover-stack.", "Complementa simulate_hover_layout_10.py."]),

n("cv-lab-restore", "cv-lab-scripts", "file", "restore_ponto_funcional_todas_telas.ps1",
  "Restaura o ponto funcional de todas as telas a partir de Veyon_Custom/_deploy.",
  "scripts/restore_ponto_funcional_todas_telas.ps1",
  "powershell -File scripts\\restore_ponto_funcional_todas_telas.ps1",
  ["Usa pacote restore-funcional-todas-telas-*.", "Não remove shared-connection se já instalado.", "Par com restore_ponto_multimonitor_funcional.ps1."]),
