...
n("cv-version-alert", "cv-root", "module", "Alerta versão incompatível",
  "Detecção de build Veyon não homologado, logs só na rede UNC e modal nas sentinelas — fechamento 08/07/2026.",
  ...
  ["Handshake envia token CimedBuildSignature no protocolo VNC.", "Qualquer PC CIMED grava na rede; só .86/.74/.94/.72 exibem modal.", "Tag: restore-versao-incompativel-funcional-20260708."]),
...
n("cv-version-compat", "cv-version-alert", "file", "CimedVersionCompatibility.cpp",
  "Filtra loopback, monta detalhes da conexão e delega gravação ao writer.",
  "core/src/CimedVersionCompatibility.cpp",
  "bool CimedVersionCompatibility::isSentinelAlertHost() {\n  return localIp == \"186.217.65.86\" || /* .74 .94 .72 */;\n}",
  ["Quatro IPs sentinelas para modal.", "PC comum (.114) só registra na rede.", "resolveHostName + lookup AD display name."]),

n("cv-version-writer", "cv-version-alert", "file", "CimedVersionAlertWriter.cpp",
  "Grava alertas somente em UNC — sem log local de versão incompatível.",
  ...
  ["alertas_versao_incompativel.log — pipe-delimited para monitor.", "acessos_veyon_versao_incompativel.log — narrativo PT.", "Dedup 5 min por fingerprint origem|user|versão|destino."]),

n("cv-version-monitor", "cv-version-alert", "file", "CimedVersionAlertMonitor.cpp",
  "Sentinelas fazem poll do log de rede a cada 2 s e disparam o presenter.",
  "core/src/CimedVersionAlertMonitor.cpp",
  "void CimedVersionAlertMonitor::startOnSentinelHosts( FeatureWorkerManager& m, QObject* parent ) {\n  if ( !isSentinelAlertHost() ) return;\n  new CimedVersionAlertMonitor( m, parent );\n}",
  ["Iniciado em ComputerControlServer::start.", "Seed inicial evita re-alertar linhas antigas.", "QTimer 2000 ms lê alertas_versao_incompativel.log."]),
