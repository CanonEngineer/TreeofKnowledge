...
n("cv-support-dialog", "cv-brand", "file", "ComputerSupportInfoDialog.cpp",
  "Diálogo F3 com informações de suporte técnico e contatos CiMED.",
  "core/src/ComputerSupportInfoDialog.cpp",
  "ComputerSupportInfoDialog::ComputerSupportInfoDialog( QWidget* parent );",
  ["Atalho F3 global (RegisterHotKey) no worker da bandeja.", "Lista hostname, IP, usuário AD.", "Chrome CIMED via CimedDialogStyle."]),
...
n("cv-tray", "cv-root", "module", "Bandeja do sistema HiDPI",
  "Ícones multi-DPI, hover CIMED, F3 suporte e servidor de senha tamper.",
...
n("cv-master", "cv-root", "module", "veyon-master",
  "Monitoramento de salas com miniaturas VNC e logs de sessão no painel CiMED.",
  "master/src/ComputerControlListModel.cpp",
  "class ComputerControlListModel : public ComputerListModel { void startComputerControlInterface(...); };",
  ["Um ComputerControlInterface por computador selecionado.", "UpdateMode::Monitoring para thumbnails.", "Integração com ComputerManager e locations salvas."]),

n("cv-monitoring-log", "cv-master", "file", "ComputerControlListModel.cpp",
  "Registra acessos do modo Monitoramento (painel com locations pré-configuradas) em acessos_veyon.",
  "master/src/ComputerControlListModel.cpp",
  "void handleMonitoringAccessLogStateChange( ComputerControlInterface* controlInterface ) {\n  if ( state == Connected ) AccessLogRecorder::logOutboundAccessSuccess(...);\n  if ( state == Disconnected ) finalizeMonitoringAccessLog(...);\n}",
  ["Handlers conectados antes de start() — evita perder Connected rápido.", "Par conexão/desconexão com duração.", "Mesmo AccessLogRecorder do Remote View/Control."]),

n("cv-monitoring-session", "cv-monitoring-log", "function", "finalizeMonitoringAccessLog()",
  "Encerra sessão de monitoramento e grava tempo de conexão no UNC.",
  "master/src/ComputerControlListModel.cpp",
  "AccessLogRecorder::logOutboundDisconnect( monitoringDestinationName(...),\n  monitoringDestinationIp(...), elapsedMs );",
  ["Chamado em Disconnected ou ao remover interface da lista.", "Estado por QHash<ComputerControlInterface*, MonitoringAccessLogState>.", "Não altera logging existente do RemoteAccessFeaturePlugin."]),

n("cv-support-hotkey", "cv-tray", "function", "setupSupportInfoHotkey()",
  "Atalho global F3 abre ComputerSupportInfoDialog na sessão do usuário.",
  "core/src/SystemTrayIcon.cpp",
  "RegisterHotKey( nullptr, SupportInfoHotkeyId, MOD_NOREPEAT, VK_F3 );\n// Linux: QShortcut( Qt::Key_F3, qApp )",
  ["Substituído F8/F9 por F3 (07/08/2026).", "SupportInfoHotkeyFilter via WM_HOTKEY.", "showSupportInfoDialog() com chrome CIMED."]),
...
n("cv-docs", "cv-deploy", "file", "docs/",
  ...
  "docs/\n  PROJETO-CIMED.md\n  RELATORIO-ESTRUTURA-CODIGO-CIMED.pdf\n  RESTORE-POINT-20260708.md",
  ["Mapa visual em PROJETO-CIMED.md.", "PDF executivo com apêndice final 08/07/2026.", "RESTORE-POINT-20260708 documenta tag funcional."]),

n("cv-version-alert", "cv-root", "module", "Alerta versão incompatível",
  "Detecção de build não homologado, gravação UNC e modal nas 4 sentinelas — fix poll UNC + multi-NIC (14/07/2026).",
  "core/src/CimedVersionCompatibility.cpp",
  "CimedVersionCompatibility::handleInboundClientSignature( client, clientBuildSignature );\nCimedVersionAlertMonitor::startOnSentinelHosts( featureWorkerManager, this );",
  ["Handshake envia token CimedBuildSignature no protocolo VNC.", "Qualquer PC CIMED grava na rede; .86/.74/.94/.72 exibem modal (qualquer NIC).", "Fix 14/07: FindFirstFile size + flush Writer; sem dedupe 5 min."]),

n("cv-build-signature", "cv-version-alert", "file", "CimedBuildSignature.cpp",
  "Token de handshake: produto|versão CIMED|versão Veyon; valida homologação.",
  "core/src/CimedBuildSignature.cpp",
  "QString CimedBuildSignature::protocolToken() {\n  return QStringLiteral(\"%1|%2\").arg(productLabel(), productVersion())\n    + QLatin1Char('|') + VeyonCore::versionString();\n}\nbool CimedBuildSignature::isHomologated( const QString& token );",
  ["Enviado no VncServerProtocol após conexão.", "Prefixo fixo Veyon CIMED|1.0|.", "describeDetectedVersion para texto do alerta."]),

n("cv-vnc-signature", "cv-build-signature", "function", "handleInboundClientSignature() hook",
  "Protocolo VNC chama compatibilidade ao receber assinatura do cliente remoto.",
  "core/src/VncServerProtocol.cpp",
  "CimedVersionCompatibility::handleInboundClientSignature( m_client, clientBuildSignature );",
...
n("cv-version-writer", "cv-version-alert", "file", "CimedVersionAlertWriter.cpp",
  ...
  "core/src/CimedVersionAlertWriter.cpp",
  "QString networkAlertDirectoryPath() {\n  return \"\\\\\\\\172.20.100.36\\\\vnc_veyon$\\\\veyon\\\\acessos_veyon_imcomp\";\n}",
  ["alertas_versao_incompativel.log — pipe-delimited para monitor.", "acessos_veyon_versao_incompativel.log — narrativo PT.", "Sem dedupe 5 min — cada handshake dispara sentinelas."]),

n("cv-version-monitor", "cv-version-alert", "file", "CimedVersionAlertMonitor.cpp",
  "Sentinelas fazem poll do log UNC (tamanho via FindFirstFile) a cada 1 s e disparam o presenter.",
  "core/src/CimedVersionAlertMonitor.cpp",
  "qint64 queryNetworkAlertFileSize(const QString& path); // FindFirstFile — evita QFile::size stale\nvoid CimedVersionAlertMonitor::startOnSentinelHosts(...);",
  ["Iniciado em ComputerControlServer::start.", "Seed inicial evita re-alertar linhas antigas.", "QTimer 1000 ms; corrige cache SMB do tamanho UNC."]),

n("cv-version-presenter", "cv-version-alert", "file", "CimedVersionAlertPresenter.cpp",
  "Roteia alerta para o worker da bandeja via FeatureMessage ShowVersionAlert.",
  "core/src/CimedVersionAlertPresenter.cpp",
  "VeyonCore::builtinFeatures().systemTrayIcon().showVersionAlert(\n  featureWorkerManager, originIp, originHost, adUser, adDisplayName, detectedVersion );",
  ["BuiltinFeatures registra presenter.", "IPC para sessão interativa do usuário.", "Não abre diálogo no processo do serviço."]),

n("cv-tray-version-alert", "cv-version-presenter", "function", "SystemTrayIcon::showVersionAlert()",
  "Worker da bandeja exibe CimedVersionAlertDialog bloqueante na sessão.",
  "core/src/SystemTrayIcon.cpp",
  "case FeatureCommand::ShowVersionAlert:\n  CimedVersionAlertDialog::showAlert( originIp, originHost, adUser, adDisplayName, detectedVersion );",
  ["ensureSessionWorker antes do sendMessage.", "qApp->setQuitOnLastWindowClosed( false ).", "Modal 680 px com texto completo da versão detectada."]),

n("cv-version-dialog", "cv-version-alert", "file", "CimedVersionAlertDialog.cpp",
...
n("cv-restore-20260708", "cv-version-alert", "file", "RESTORE-POINT-20260708.md",
  ...
  "docs/RESTORE-POINT-20260708.md",
...
n("cv-release-github", "cv-deploy", "file", "GitHub Release 1.0 win64",
  "Instaladores lab + posto (07/08/2026): logs Monitoramento + F3 suporte.",
  ...
  ["Veyon-CIMED-1.0-win64-setup.exe e -posto-setup.exe (~44 MB).", "Log de Monitoramento via ComputerControlListModel.", "Atalho suporte F3; NSIS /INPUTCHARSET UTF8."]),

n("cv-release", "cv-root", "module", "Release Git",
  "Ramificação separada para versionamento semver e publicação de tags no GitHub.",
  "tag.bat",
  "@echo off\n:: GERADOR DE RELEASE DO PROJETO — tag.bat",
  ["Execute na raiz do repositório CustomizeVeyonProject.", "Exige working tree limpa, branch main e acesso ao origin.", "Escolha Major, Minor ou Patch para calcular a próxima tag v*."]),

n("cv-tag-bat", "cv-release", "file", "tag.bat",
  "Gerador interativo de release: valida Git, atualiza main, calcula semver e publica a tag.",
  "tag.bat",
  "@echo off\nchcp 65001 > nul\necho GERADOR DE RELEASE DO PROJETO",
  ["Duplo clique ou execute tag.bat na raiz do projeto.", "Confirme a nova versão quando solicitado.", "Use a tag criada para releases e restore points no GitHub."]),
]
...
PROJECT = {
    ...
    "summary": "Veyon CIMED/HCFMB — UI HiDPI, logs SHA-256 em UNC (Remote View + Monitoramento), F3 suporte, anti-tampering e deploy portátil.",
    "nodes": NODES,
}
